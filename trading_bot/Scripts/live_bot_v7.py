...
MIN_QUOTE_VOLUME = 10000000 # [Ver.7] 최소 24시간 거래대금 (10M USDT)
...
def check_api_connection():
    try:
        ...
        logging.info("✅ [Ver.7] 동적 지정가 매매 + MTF 타겟팅 + 유동성 필터(Anti-Slippage) 가동 시작")
        logging.info("-" * 60)
...
def analyze_symbol(symbol):
    """지표, 손익비 및 유동성을 분석하여 진입 여부를 결정합니다."""
    ...
    if s1_pass and s2_pass and s3_pass:
        ...
        # [Ver.7 추가] 유동성 필터: 24시간 거래대금 검사 (슬리피지 방지)
        try:
            ticker = exchange.fetch_ticker(symbol)
            quote_vol = ticker.get('quoteVolume', 0) 
            
            if quote_vol < MIN_QUOTE_VOLUME:
                logging.info(f"🚫 [{symbol}] 진입 포기 (거래대금 미달): 24H Vol {quote_vol/1000000:.1f}M USDT (Low Liquidity)")
